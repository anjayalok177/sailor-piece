...
return function(lib, sideData, contentArea, bgF, root, rootCorner,
    ...
    -- subPages[name] = plain Frame (lihat mkSubTabBar)
    local subPages=mkSubTabBar(mainInner,{"Farm","TP","Boss","Dungeon"})

    -- ════════════════════════════════
    -- FARM — dua kolom
    -- Kiri: kontrol | Kanan: slider adjust
    -- ════════════════════════════════
    -- mkTwoColLayout sekarang 1 SF + 2 plain Frame kolom
    ...
    -- Status label di atas (shared, di dalam outer frame sebelum SF dibuat)
    -- Karena mkTwoColLayout sudah membungkus segalanya dalam SF,
    -- kita tambahkan status di dalam salah satu kolom sebagai item pertama.
    ...
    -- Placeholder di kolom kanan agar simetris
    ...
    -- 8 di kiri (index 1-8), 8 di kanan (index 9-16)
    ...
    -- Status + ON/OFF di kolom kiri baris pertama
    ...
    -- List boss di kolom kiri (boss 1-5) dan kanan (boss 6-9)
    local bossListGroup=mkGroupBox(bossLeftF,2)
    mkSectionLabel(bossListGroup,"Pilih Boss",1)

    local bossRightGroup=mkGroupBox(bossRightF,1)
    mkSectionLabel(bossRightGroup,"—",1)

    local selectedBoss=nil
    local bossCards={}

    local function makeBossCard(parent,bossName,idx,order)
    ...
    function rebuildBossCards()
        for _,c in ipairs(bossCards) do pcall(function() c:Destroy() end) end
        bossCards={}
        -- Boss 1-5 di grup kiri, 6-9 di grup kanan
        for idx,bossName in ipairs(KNOWN_BOSSES) do
            ...
            local c=makeBossCard(parent,bossName,idx,order)
            table.insert(bossCards,c)
        end
    end
    rebuildBossCards()
    ...
    return {
        ...
        -- No-op stubs (farm no longer has dedicated status labels)

...
return function(refs,T)
    ...
    -- FIX Bug#7: multi-path discovery + recursive fallback
    local hitRemote=nil
    local function getHitRemote()
        if hitRemote and hitRemote.Parent then return hitRemote end
        local paths={{"CombatSystem","Remotes","RequestHit"},{"Remotes","RequestHit"},{"Combat","RequestHit"}}
        ...
        local f=ReplicatedStorage:FindFirstChild("RequestHit",true); if f then hitRemote=f end
        return hitRemote
    end
    ...
    local function getAbilityRemote()
        ...
        local paths={{"AbilitySystem","Remotes","RequestAbility"},{"Remotes","RequestAbility"},{"Ability","RequestAbility"}}
        for _,path in ipairs(paths) do
            local ok,r=pcall(function()
                local n=ReplicatedStorage; for _,s in ipairs(path) do n=n:WaitForChild(s,2) end; return n
            end)
        ...
        local f=ReplicatedStorage:FindFirstChild("RequestAbility",true); if f then abilityRemote=f end
        return abilityRemote
    end

    local flyBP,flyBG=nil,nil

    -- FIX Bug#3: spin has no gap when active — task.wait(0.3) only when spin is OFF
    task.spawn(function()
        while true do
            if refs and refs.getSpinOn and refs.getSpinOn() then
                ...
                if r then
                    ...
                    -- no extra wait here when spin is ON
                else
                    task.wait(0.1)
                end
            else
                task.wait(0.3) -- only wait when spin is OFF
            end
        end
    end)
    ...
    local function enableFly()
        ...
        flyBP=Instance.new("BodyPosition"); flyBP.MaxForce=Vector3.new(1e5,1e5,1e5); flyBP.D=500; flyBP.P=5000
        flyBP.Position=r.Position; flyBP.Parent=r
        flyBG=Instance.new("BodyGyro"); flyBG.MaxTorque=Vector3.new(1e5,1e5,1e5); flyBG.D=400
    ...
    local function startHitSpam()
        if hitRunning then return end; hitRunning=true
        task.spawn(function()
            local remote=getHitRemote(); if not remote then hitRunning=false; return end
            while hitRunning do pcall(function() remote:FireServer() end); task.wait() end
        end)
    end
    ...
    local function startAbilityLoop(checkFn)
        task.spawn(function()
            ...
            while checkFn() do
                if remote then for _,arg in ipairs({1,2,3}) do if not checkFn() then break end; pcall(function() remote:FireServer(arg) end) end end
                task.wait(0.5)
            end
        end)
    end

    -- FIX Bug#13: guard refs before accessing
    task.spawn(function()
        ...
        for i=1,19 do
            ...
            if npc then
                ...
                if pos and (r.Position-pos).Magnitude<=100 then
                    if name~=lastQuestNPC then fireQuestAccept(name); fireSettings("EnableQuestRepeat",true); fireSettings("AutoQuestRepeat",true); lastQuestNPC=name end
                    return
                end
            end
        end
    ...
    local function farmTeardown()
        ...
        if not (refs and refs.getAutoHitOn and refs.getAutoHitOn()) then stopHitSpam() end
    end

    -- FIX Bug#5: check state BEFORE farmSetup
    local function farmLoopV1()
        isRunningV1=true
        if not farmV1On then isRunningV1=false; if refs then refs.setFarmOnOff(false) end; return end
        ...
        stopHitSpam(); farmTeardown(); _G.islandFarmOn=false; isRunningV1=false
        if refs then refs.setFarmOnOff(false) end
    end

    local function farmLoopV2()
        isRunningV2=true
        if not farmV2On then isRunningV2=false; if refs then refs.setFarmOnOff(false) end; return end
        _G.islandFarmOn=true
        local island=refs.getIsland(); farmSetup(island)
        if refs.getAutoHitOn and refs.getAutoHitOn() then startHitSpam() end
        ...
        while farmV2On do
            ...
            moveTo(fp,refs.getSpeed()); tryAutoQuest()
            local ld=refs.getLD()
            if ld>0 then local endT=tick()+ld; while tick()<endT and farmV2On do tryAutoQuest(); task.wait(0.5) end
            else task.wait(0.5) end
        end
        stopHitSpam(); farmTeardown(); _G.islandFarmOn=false; isRunningV2=false
        if refs then refs.setFarmOnOff(false) end
    end
    ...
    local function enableBossFly()
        local r=getRoot(); local h=character and character:FindFirstChildOfClass("Humanoid")
        if not r or not h then return end; h.PlatformStand=true
        if bossFlyBP then bossFlyBP:Destroy() end; if bossFlyBG then bossFlyBG:Destroy() end
        bossFlyBP=Instance.new("BodyPosition"); bossFlyBP.MaxForce=Vector3.new(1e5,1e5,1e5); bossFlyBP.D=600; bossFlyBP.P=6000
        bossFlyBP.Position=r.Position; bossFlyBP.Parent=r
        bossFlyBG=Instance.new("BodyGyro"); bossFlyBG.MaxTorque=Vector3.new(1e5,1e5,1e5); bossFlyBG.D=500
    ...
    -- FIX Bug#8: broader search for boss NPC
    ...
    local function bossKillLoop()
        ...
        if not data then refs.setBossStat("Data tidak ditemukan!",T.red); refs.setBossOnOff(false); bossKillOn=false; return end
        ...
        local r=getRoot()
        if r then r.CFrame=CFrame.new(bossCoord+Vector3.new(0,5,0)); if bossFlyBP then bossFlyBP.Position=bossCoord+Vector3.new(0,5,0) end end
        task.wait(0.5)
        local bossHitRun=true
        task.spawn(function()
            local remote=getHitRemote()
            while bossHitRun and bossKillOn do if remote then pcall(function() remote:FireServer() end) end; task.wait() end
        end)
        refs.setBossPhase("Menyerang",T.green)
        while bossKillOn do
            local bossPart=getBossPart(npcName)
            if not bossPart then refs.setBossStat(npcName.." selesai!",T.green); refs.setBossPhase("Boss hilang",T.green); break end
            ...
            if dist>4 then TweenService:Create(rr,TweenInfo.new(math.clamp(dist/70,0.05,0.6),Enum.EasingStyle.Linear),{CFrame=CFrame.new(target)}):Play() end
            task.wait(0.15)
        end
    ...
    local function findNearestNPC(folder)
        ...
        for _,npc in ipairs(folder:GetChildren()) do
            ...
            if part then local d=(r.Position-part.Position).Magnitude; if d<bestDist then bestDist=d; best=npc end end
        end
        return best
    end
    local function dungeonLoop()
        ...
        task.spawn(function()
            local remote=getHitRemote()
            if not remote then refs.setDungeonStat("Remote tidak ditemukan!",T.red); dungeonHitRun=false; dungeonOn=false; refs.setDungeonOnOff(false); return end
            local count=0; local lastT=tick()
            while dungeonHitRun do
                pcall(function() remote:FireServer() end); count=count+1
                if tick()-lastT>=0.5 then refs.setDungeonHit(tostring(count*2).."/s",T.green); count=0; lastT=tick() end
                task.wait()
            end
        ...
        task.spawn(function()
            while dungeonOn do
                ...
                if not folder or #folder:GetChildren()==0 then refs.setDungeonStat("Tidak ada NPC",T.textDim); task.wait(0.5); continue end
                ...
                local elapsed=0
                while elapsed<1 and dungeonOn and npc and npc.Parent do task.wait(0.1); elapsed=elapsed+0.1 end
    ...
    refs.setFarmCallback(function(v)
        ...
        else farmV1On=false; farmV2On=false; stopHitSpam() end
    end)

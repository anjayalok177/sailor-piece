...
snd.SoundId=“rbxassetid://81906781454850”
...
tl.BackgroundTransparency=1; tl.Text=title; tl.TextColor3=T.white
tl.Font=Enum.Font.GothamBold; tl.TextSize=13
tl.TextXAlignment=Enum.TextXAlignment.Left; tl.ZIndex=601
...
sl.TextXAlignment=Enum.TextXAlignment.Left; sl.ZIndex=601
...
irBtn.Font=Enum.Font.GothamBold; irBtn.TextSize=11
irBtn.BorderSizePixel=0; irBtn.LayoutOrder=2; irBtn.ZIndex=6
    ...
    for _,child in ipairs(workspace:GetChildren()) do
        ...
        if bossName then
            ...
            table.insert(timerEntries,{
                container=child, bossName=bossName, timerLbl=timerLbl,
                dispTimer=dispTimer, dispStatus=dispStatus,
                cardStroke=cs, accentBar=abar, prevSecs=-1
            })
        end
    end
    if found==0 then
        ...
        el.Text="Tidak ada TimedBossSpawn"; el.TextColor3=T.textDim
        el.Font=Enum.Font.Gotham; el.TextSize=10; el.LayoutOrder=1
    end
end
        ...
        for _,e in ipairs(timerEntries) do
            pcall(function()
                if not e.timerLbl or not e.timerLbl.Parent then
                    ...
                    if f then
                        e.timerLbl=f
                        e.dispStatus.Text="Timer OK"; e.dispStatus.TextColor3=T.green
                    else
                        e.dispTimer.Text="?"
                        e.dispStatus.Text="Belum ada timer"; e.dispStatus.TextColor3=T.amber
                        return
                    end
                end
...
-- Status label di atas kolom kiri
...
local function makeTpCard(parent,loc,order)
    ...
    ibar.BorderSizePixel=0; Instance.new("UICorner",ibar).CornerRadius=UDim.new(1,0)
...
-- LEFT: Kill Boss (satu boss dipilih)
...
activeBossContainer.BorderSizePixel=0; activeBossContainer.LayoutOrder=5
local abcL=Instance.new("UIListLayout",activeBossContainer)
abcL.Padding=UDim.new(0,3); abcL.SortOrder=Enum.SortOrder.LayoutOrder
...
local function rebuildActiveBossCards()
    ...
    if #active==0 then
        ...
        el.Size=UDim2.new(1,0,0,22); el.LayoutOrder=1
        el.BackgroundTransparency=1; el.Text="Tidak ada boss aktif"
    ...
    for idx,bossName in ipairs(active) do
        ...
        adot.Size=UDim2.new(0,5,0,5); adot.Position=UDim2.new(0,7,0.5,0)
        adot.AnchorPoint=Vector2.new(0,0.5); adot.BackgroundColor3=T.green
        adot.BorderSizePixel=0; Instance.new("UICorner",adot).CornerRadius=UDim.new(1,0)
        local nameL=Instance.new("TextLabel",card)
        nameL.Size=UDim2.new(1,-46,1,0); nameL.Position=UDim2.new(0,17,0,0)
        ...
        nameL.TextSize=9; nameL.TextXAlignment=Enum.TextXAlignment.Left; nameL.ZIndex=6
        local selBtn=Instance.new("TextButton",card)
        selBtn.Size=UDim2.new(0,34,0,16); selBtn.Position=UDim2.new(1,-38,0.5,0)
        selBtn.AnchorPoint=Vector2.new(0,0.5)
        selBtn.BackgroundColor3=isSel and T.accentSoft or Color3.fromRGB(22,20,36)
        selBtn.Text=isSel and "ON" or "Set"; selBtn.TextColor3=T.white
        selBtn.Font=Enum.Font.GothamBold; selBtn.TextSize=8; selBtn.BorderSizePixel=0; selBtn.ZIndex=7
...
-- RIGHT: Auto Kill Boss (multi-select)
...
for idx,bossName in ipairs(KNOWN_BOSSES) do
    ...
    dot.Size=UDim2.new(0,5,0,5); dot.Position=UDim2.new(0,8,0.5,0)
    ...
    local nameL=Instance.new("TextLabel",card)
    nameL.Size=UDim2.new(1,-36,1,0); nameL.Position=UDim2.new(0,18,0,0)
    nameL.BackgroundTransparency=1; nameL.Text=bossName; nameL.TextColor3=T.textSub
    nameL.Font=Enum.Font.Gotham; nameL.TextSize=9
    nameL.TextXAlignment=Enum.TextXAlignment.Left; nameL.ZIndex=6
    local chk=Instance.new("TextLabel",card)
    chk.Size=UDim2.new(0,18,1,0); chk.Position=UDim2.new(1,-20,0,0)
    chk.BackgroundTransparency=1; chk.Text=""; chk.TextColor3=T.green
    chk.Font=Enum.Font.GothamBold; chk.TextSize=12; chk.ZIndex=7
...
-- ═══════════════════════════════════════════
-- SETTINGS PAGE — dua subtab: Tampilan | Webhook
-- ═══════════════════════════════════════════
...
-- FIX: ambil ukuran root saat callback dipanggil, bukan saat inisialisasi
-- (mencegah 0,0 jika root belum di-render penuh)
...
searchRow.BorderSizePixel=0; searchRow.LayoutOrder=5; searchRow.ZIndex=5
...
searchBox.Size=UDim2.new(1,-76,1,-8); searchBox.Position=UDim2.new(0,10,0,4)
searchBox.BackgroundTransparency=1; searchBox.Text=""
searchBox.PlaceholderText="Cari item..."; searchBox.TextColor3=T.text
searchBox.PlaceholderColor3=T.textDim; searchBox.Font=Enum.Font.Gotham
searchBox.TextSize=10; searchBox.TextXAlignment=Enum.TextXAlignment.Left
searchBox.ClearTextOnFocus=false; searchBox.ZIndex=6

local function mkSmallBtn(parent,xOff,bg,sym,col)
    local b=Instance.new("TextButton",parent)
    b.Size=UDim2.new(0,28,0,22); b.Position=UDim2.new(1,xOff,0.5,0)
    b.AnchorPoint=Vector2.new(0,0.5); b.BackgroundColor3=bg
    b.Text=sym; b.TextColor3=col or T.white; b.Font=Enum.Font.GothamBold
    b.TextSize=12; b.BorderSizePixel=0; b.ZIndex=7
...
-- FIX: LayoutOrder 6 → komentar label jumlah item
...
selCountLbl.Text="Belum ada item dipilih"; selCountLbl.TextColor3=T.textDim
selCountLbl.Font=Enum.Font.Gotham; selCountLbl.TextSize=9
selCountLbl.TextXAlignment=Enum.TextXAlignment.Right
selCountLbl.LayoutOrder=6; selCountLbl.ZIndex=5

local itemListSF=Instance.new("ScrollingFrame",webhookSF)
itemListSF.Size=UDim2.new(1,0,0,164)
itemListSF.BackgroundTransparency=1; itemListSF.BorderSizePixel=0
itemListSF.ScrollBarThickness=2; itemListSF.ScrollBarImageColor3=T.accent
...
itemListSF.ZIndex=3; itemListSF.ClipsDescendants=true
...
local function loadStorageNames()
    local names={}
    -- FIX: pcall lebih ketat, cek tiap level sebelum lanjut
    local ok,storage=pcall(function()
        local pg=LocalPlayer:FindFirstChild("PlayerGui"); if not pg then error("no PlayerGui") end
...
local function rebuildItemList(filter)
    ...
    for _,itemName in ipairs(cachedItemNames) do
        if filterLow=="" or itemName:lower():find(filterLow,1,true) then
            ...
            local dot=Instance.new("Frame",card)
            dot.Size=UDim2.new(0,4,0,4); dot.Position=UDim2.new(0,7,0.5,0)
            dot.AnchorPoint=Vector2.new(0,0.5)
            dot.BackgroundColor3=isSel and T.green or T.textDim
            dot.BorderSizePixel=0; Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)
            local nameL=Instance.new("TextLabel",card)
            nameL.Size=UDim2.new(1,-36,1,0); nameL.Position=UDim2.new(0,16,0,0)
            nameL.BackgroundTransparency=1; nameL.Text=itemName
            nameL.TextColor3=isSel and T.white or T.textSub
            nameL.Font=isSel and Enum.Font.GothamBold or Enum.Font.Gotham
            nameL.TextSize=9; nameL.TextXAlignment=Enum.TextXAlignment.Left; nameL.ZIndex=6
            local chk=Instance.new("TextLabel",card)
            chk.Size=UDim2.new(0,18,1,0); chk.Position=UDim2.new(1,-20,0,0)
            chk.BackgroundTransparency=1; chk.Text=isSel and "✓" or ""
            chk.TextColor3=T.green; chk.Font=Enum.Font.GothamBold
            chk.TextSize=11; chk.ZIndex=7
            local hit=Instance.new("TextButton",card)
            hit.Size=UDim2.new(1,0,1,0); hit.BackgroundTransparency=1; hit.Text=""; hit.ZIndex=8
            local ci=itemName; local ccard=card; local ccs=cs
    ...
    if order==0 then
        ...
        el.Size=UDim2.new(1,-2,0,26); el.BackgroundTransparency=1
        el.LayoutOrder=1
...
return {
    ...
    setAutoBossOnOff=setAutoBossOnOff, getAutoBossOn=getAutoBossOn,

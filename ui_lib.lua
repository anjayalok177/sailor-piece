...
local UISettings = {
    scale=1.0, accentPreset="Purple", cornerRadius=14,
    particles=true, particleCount=26, glow=true,
    fontSize=12, miniBgMode="Solid", uiBgMode="Solid",
}

-- Font size registry
local fontSizeReg = {}
local function regFS(obj) if obj then table.insert(fontSizeReg,obj) end end
local function applyFontSize(v)
    UISettings.fontSize=v
    for _,o in ipairs(fontSizeReg) do
        pcall(function() if o and o.Parent then o.TextSize=v end end)
    end
end

-- Accent registry
local accentReg = {}
local function regAccent(t,o) table.insert(accentReg,{t=t,o=o}) end
local function applyAccentLive()
    for _,e in ipairs(accentReg) do
        pcall(function()
            local o,t=e.o,e.t
...
-- =====================
-- TWO-COLUMN LAYOUT
-- FIX KRITIS: Satu ScrollingFrame + dua plain Frame kolom.
-- Dua ScrollingFrame berdampingan = input engine Roblox tidak
-- bisa menentukan target → scroll & klik keduanya gagal total.
-- =====================
local function mkTwoColLayout(parent)
    -- Satu SF tunggal sebagai scroll container
    local sf=Instance.new("ScrollingFrame",parent)
    sf.Size=UDim2.new(1,0,1,0)
    sf.BackgroundTransparency=1; sf.BorderSizePixel=0
    sf.ScrollBarThickness=2; sf.ScrollBarImageColor3=T.accent
    ...
    -- Padding wrapper supaya ada jarak dari tepi
    local pp=Instance.new("UIPadding",sf)
    ...
    leftF.Size=UDim2.new(0.5,-4,0,0)
    ...
    rightF.Size=UDim2.new(0.5,-4,0,0)
    rightF.Position=UDim2.new(0.5,4,0,0)
...
local function mkSectionLabel(parent,label,order)
    ...
    t.TextColor3=T.textSub; t.Font=Enum.Font.GothamBold; t.TextSize=8
...
local function mkSection(parent,label,order)
    ...
    t.BackgroundTransparency=1; t.Text=string.upper(label)
    t.TextColor3=T.textSub; t.Font=Enum.Font.GothamBold; t.TextSize=9
    t.TextXAlignment=Enum.TextXAlignment.Left
    return f
end
...
local function mkOnOffBtn(parent,label,order)
    ...
    local function applyVisual(v)
        if v then
            ...
            smooth(wStroke,{Color=T.green,Transparency=0,Thickness=1.4},0.18):Play()
...
-- Dropdown V2 (setOpen didefinisikan SEBELUM item loop)
local function mkDropdownV2(parent,label,icon,iconCol,items,default,onChange,order)
    ...
    -- setOpen SEBELUM item loop agar auto-close berfungsi
...
-- =====================
-- SUB-TAB BAR
-- FIX: Returns subPages[name] = plain Frame.
-- Caller menentukan layout di dalamnya (mkScrollPage atau mkTwoColLayout).
-- =====================
local function mkSubTabBar(parent,tabs)
    ...
    for i,name in ipairs(tabs) do
        ...
        -- plain Frame — bukan {sf,frame} — caller tentukan layout

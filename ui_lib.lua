...
local T = {
    ...
    card         = Color3.fromRGB(15,  14,  24),
    cardHover    = Color3.fromRGB(22,  20,  36),
    border       = Color3.fromRGB(40,  34,  68),
    borderBright = Color3.fromRGB(80,  65, 130),
    accent       = Color3.fromRGB(118,  68, 255),
    accentSoft   = Color3.fromRGB(88,   48, 188),
    accentGlow   = Color3.fromRGB(152,  92, 255),
    accentDim    = Color3.fromRGB(55,   30, 120),
    ...
    textSub      = Color3.fromRGB(130, 122, 165),
    textDim      = Color3.fromRGB(62,   55,  92),
    white        = Color3.fromRGB(255, 255, 255),
    black        = Color3.fromRGB(8,    7,  14),
...
local function ripple(parent,x,y,col)
    ...
    r.BackgroundTransparency=0.75; r.BorderSizePixel=0; r.ZIndex=60; r.Parent=parent
    Instance.new("UICorner",r).CornerRadius=UDim.new(1,0)
    local sz=math.max(parent.AbsoluteSize.X,parent.AbsoluteSize.Y)*2.4
    local t1=smooth(r,{Size=UDim2.new(0,sz,0,sz),BackgroundTransparency=0.94},0.42)
    t1:Play()
    t1.Completed:Connect(function()
        smooth(r,{BackgroundTransparency=1},0.14):Play()
        task.wait(0.16); pcall(function() r:Destroy() end)
    end)
end
...
local function mkGroupBox(parent, order)
    ...
    grp.BackgroundColor3=Color3.fromRGB(12,11,20)
    ...
    gs.Color=T.borderBright; gs.Thickness=1.2; gs.Transparency=0.12
...
local function mkSection(parent,label,order)
    ...
    hline.BackgroundColor3=T.border; hline.BackgroundTransparency=0.1
...
local function mkCard(parent,h,order)
    ...
    cs.Color=T.border; cs.Transparency=0.3; cs.Thickness=1.0
    ...
    c.MouseEnter:Connect(function()
        if c and c.Parent then
            smooth(c,{BackgroundColor3=T.cardHover},0.14):Play()
            smooth(cs,{Color=T.accentGlow,Transparency=0.15},0.14):Play()
        end
    end)
    c.MouseLeave:Connect(function()
        if c and c.Parent then
            smooth(c,{BackgroundColor3=T.card},0.14):Play()
            smooth(cs,{Color=T.border,Transparency=0.3},0.14):Play()
        end
    end)
    return c,cs
end
...
local function mkStatus(parent,prefix,init,order)
    local c=Instance.new("Frame",parent)
    c.Size=UDim2.new(1,0,0,30); c.BackgroundColor3=Color3.fromRGB(14,13,22)
    c.BorderSizePixel=0; c.LayoutOrder=order or 0; c.ZIndex=5
    Instance.new("UICorner",c).CornerRadius=UDim.new(0,8)
    local dot=Instance.new("Frame",c)
    dot.Size=UDim2.new(0,5,0,5); dot.Position=UDim2.new(0,10,0.5,0)
    ...
    pfx.Size=UDim2.new(0,52,1,0); pfx.Position=UDim2.new(0,20,0,0)
    ...
    val.Size=UDim2.new(1,-78,1,0); val.Position=UDim2.new(0,74,0,0)
    ...
    local function set(text,col)
        val.Text=text or "--"
        if col then
            smooth(val,{TextColor3=col},0.18):Play()
            smooth(dot,{BackgroundColor3=col},0.18):Play()
        end
    end
    return c,set
end
...
local function mkToggle(parent,label,default,onChange,order)
    ...
    lbl.Size=UDim2.new(1,-66,1,0); lbl.Position=UDim2.new(0,12,0,0)
    ...
    track.Size=UDim2.new(0,36,0,18); track.Position=UDim2.new(1,-44,0.5,0)
    ...
    local function apply(v)
        val=v
        smooth(track,{BackgroundColor3=v and T.accent or T.border},0.2):Play()
        smooth(thumb,{Position=UDim2.new(v and 1 or 0,v and -15 or 3,0.5,0)},0.2):Play()
...
local function mkSlider(parent,label,min,max,default,suffix,onChange,order)
    ...
    local lbl=Instance.new("TextLabel",card)
    lbl.Size=UDim2.new(0.62,0,0,18); lbl.Position=UDim2.new(0,12,0,6)
    lbl.BackgroundTransparency=1; lbl.Text=label; lbl.TextColor3=T.text
    lbl.Font=Enum.Font.GothamBold; lbl.TextSize=UISettings.fontSize
    lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=6
    regFS(lbl)
    local valLbl=Instance.new("TextLabel",card)
    valLbl.Size=UDim2.new(0.38,-12,0,18); valLbl.Position=UDim2.new(0.62,0,0,6)
    ...
    trackBg.Size=UDim2.new(1,-24,0,4); trackBg.Position=UDim2.new(0,12,0,42)
    ...
    local ks=Instance.new("UIStroke",knob); ks.Color=T.accent; ks.Thickness=1.8
    ...
    local function setVal(v)
        ...
        smooth(fill,{Size=UDim2.new(r,0,1,0)},0.08):Play()
        smooth(knob,{Position=UDim2.new(r,0,0.5,0)},0.08):Play()
        if onChange then onChange(curVal) end
    end
    setVal(curVal)
    local sHit=Instance.new("TextButton",card)
    sHit.Size=UDim2.new(1,-20,0,28); sHit.Position=UDim2.new(0,10,0,30)
    sHit.BackgroundTransparency=1; sHit.Text=""; sHit.ZIndex=12
    sHit.MouseButton1Down:Connect(function()
        dragging=true; spring(knob,{Size=UDim2.new(0,17,0,17)},0.18):Play()
    end)
    ...
    UIS.InputEnded:Connect(function(i)
        ...
        or i.UserInputType==Enum.UserInputType.Touch then
            if dragging then dragging=false; spring(knob,{Size=UDim2.new(0,13,0,13)},0.22):Play() end
        end
    end)
...
local function mkOnOffBtn(parent,label,order)
    ...
    local function applyVisual(v)
        if v then
            ...
            smooth(wStroke,{Color=T.green,Transparency=0.0,Thickness=1.5},0.20):Play()
            smooth(lbl,{TextColor3=T.black},0.20):Play()
            smooth(dot,{BackgroundColor3=T.green},0.20):Play()
            smooth(dotGlow,{Color=T.green},0.20):Play()
            smooth(wGlow,{ImageTransparency=0.74},0.28):Play()
            statusTxt.Text="ON"
            smooth(statusTxt,{TextColor3=Color3.fromRGB(22,22,22)},0.1):Play()
        else
            smooth(wrapper,{BackgroundColor3=Color3.fromRGB(14,12,22)},0.20):Play()
            smooth(wStroke,{Color=T.border,Transparency=0.2,Thickness=1.2},0.20):Play()
            smooth(lbl,{TextColor3=T.textSub},0.20):Play()
            smooth(dot,{BackgroundColor3=T.red},0.20):Play()
            smooth(dotGlow,{Color=T.red},0.20):Play()
            smooth(wGlow,{ImageTransparency=1},0.20):Play()
            statusTxt.Text="OFF"
    ...
    hit.MouseEnter:Connect(function()
        if not on then smooth(wrapper,{BackgroundColor3=Color3.fromRGB(20,18,32)},0.12):Play() end
    end)
    hit.MouseLeave:Connect(function()
        if not on then smooth(wrapper,{BackgroundColor3=Color3.fromRGB(14,12,22)},0.12):Play() end
    end)
    hit.MouseButton1Click:Connect(function()
        on=not on
    ...
    task.spawn(function()
        while wrapper and wrapper.Parent do
            if on then
                ease(wGlow,{ImageTransparency=0.64},0.68):Play(); task.wait(0.80)
                ease(wGlow,{ImageTransparency=0.82},0.68):Play(); task.wait(0.80)
...
local function mkDropdownV2(parent,label,icon,iconCol,items,default,onChange,order)
    ...
    local wStroke=Instance.new("UIStroke",wrapper)
    wStroke.Color=T.border; wStroke.Transparency=0.3; wStroke.Thickness=1.0
    ...
    for idx,item in ipairs(items) do
        ...
        hlBg.BackgroundTransparency=(item==selected) and 0.75 or 1
        ...
        itemBtn.MouseEnter:Connect(function()
            if item~=selected then
                ...
                smooth(itemTxt,{TextColor3=T.text},0.10):Play()
            end
        end)
        itemBtn.MouseLeave:Connect(function()
            if item~=selected then
                smooth(hlBg,{BackgroundTransparency=1},0.10):Play()
        ...
        itemBtn.MouseButton1Click:Connect(function()
            ...
            for i2,d in ipairs(itemFrames) do
                ...
                smooth(d.hlBg,{BackgroundTransparency=isSel and 0.75 or 1},0.16):Play()
                smooth(d.dot,{BackgroundColor3=isSel and T.accentGlow or T.textDim,
                    BackgroundTransparency=isSel and 0 or 0.6},0.16):Play()
                smooth(d.txt,{TextColor3=isSel and T.white or T.textSub},0.16):Play()
    ...
    header.MouseEnter:Connect(function()
        if not open then smooth(wrapper,{BackgroundColor3=T.cardHover},0.12):Play() end
    end)
...
local function mkSubTabBar(parent,tabs)
    ...
    barContainer.BackgroundColor3=Color3.fromRGB(10,9,18)
    ...
    bcStroke.Color=T.border; bcStroke.Transparency=0.1; bcStroke.Thickness=1.2
    for i=1,#tabs-1 do
        local ts=Instance.new("Frame",barContainer)
        ts.Size=UDim2.new(0,1,0.5,0); ts.Position=UDim2.new(i/#tabs,0,0.25,0)
        ts.BackgroundColor3=T.border; ts.BackgroundTransparency=0.3
    ...
    sep.BackgroundColor3=T.borderBright; sep.BackgroundTransparency=0.25
    ...
    local tabTextSize=#tabs>=6 and 9 or 10
    local subBtns={}; local subPages={}
    local function switchSub(idx)
        smooth(pill,{Position=UDim2.new((idx-1)/#tabs,3,0,4)},0.24):Play()
        for i2,d in ipairs(subBtns) do
            smooth(d.lbl,{TextColor3=(i2==idx) and T.white or T.textSub},0.20):Play()

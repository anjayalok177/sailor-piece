...
local UISettings={
    scale=1,accentPreset="Purple",cornerRadius=14,particles=true,
...
local function mkCard(parent,h,order)
    ...
    c.MouseEnter:Connect(function()
        if c and c.Parent then smooth(c,{BackgroundColor3=T.cardHover},0.12):Play(); smooth(cs,{Color=T.borderBright,Transparency=0.2},0.12):Play() end
    end)
    c.MouseLeave:Connect(function()
        if c and c.Parent then smooth(c,{BackgroundColor3=T.card},0.12):Play(); smooth(cs,{Color=T.border,Transparency=0.45},0.12):Play() end
    end)
    return c,cs
end
...
local function mkToggle(parent,label,default,onChange,order)
    ...
    lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=6; regFS(lbl)
    ...
    hit.MouseButton1Click:Connect(function()
        ripple(card,card.AbsoluteSize.X*0.5,card.AbsoluteSize.Y*0.5,T.accent); apply(not val)
    end)
...
local function mkSlider(parent,label,min,max,default,suffix,onChange,order)
    ...
    fill.Size=UDim2.new(0,0,1,0); fill.BackgroundColor3=T.accent; fill.BorderSizePixel=0; fill.ZIndex=7
    Instance.new("UICorner",fill).CornerRadius=UDim.new(1,0)
    Instance.new("UIGradient",fill).Color=ColorSequence.new{
        ColorSequenceKeypoint.new(0,T.accentGlow),ColorSequenceKeypoint.new(1,T.accent),
...
local function mkOnOffBtn(parent,label,order)
    ...
    wGlow.Image="rbxassetid://5028857084"; wGlow.ImageColor3=T.green; wGlow.ImageTransparency=1; wGlow.ZIndex=0
    ...
    lbl.Font=Enum.Font.GothamBold; lbl.TextSize=UISettings.fontSize; lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=8
    regFS(lbl)
    ...
    task.spawn(function()
        while wrapper and wrapper.Parent do
            if on then ease(wGlow,{ImageTransparency=0.66},0.7):Play(); task.wait(0.82)
...
local function mkDropdownV2(parent,label,icon,iconCol,items,default,onChange,order)
    ...
    Instance.new("UIGradient",wrapper).Color=ColorSequence.new{
        ColorSequenceKeypoint.new(0,Color3.fromRGB(22,20,34)),ColorSequenceKeypoint.new(1,Color3.fromRGB(14,13,22)),
    }
    local header=Instance.new("TextButton",wrapper)
    header.Size=UDim2.new(1,0,0,HEADER_H); header.BackgroundTransparency=1; header.Text=""; header.ZIndex=8
    ...
    icSym.Size=UDim2.new(1,0,1,0); icSym.BackgroundTransparency=1; icSym.Text=icon or "?"
    ...
    labelL.TextXAlignment=Enum.TextXAlignment.Left; labelL.ZIndex=9; regFS(labelL)
    ...
    arrowL.BackgroundTransparency=1; arrowL.Text="v"; arrowL.TextColor3=T.textSub
    arrowL.Font=Enum.Font.GothamBold; arrowL.TextSize=10; arrowL.ZIndex=9
    local selected=default or items[1]; local itemFrames={}
    local open=false
    ...
    for idx,item in ipairs(items) do
        ...
        hlBg.BackgroundColor3=T.accent; hlBg.BackgroundTransparency=(item==selected) and 0.78 or 1
        ...
        checkL.BackgroundTransparency=1; checkL.Text=(item==selected) and "v" or ""
        checkL.TextColor3=T.accentGlow; checkL.Font=Enum.Font.GothamBold; checkL.TextSize=10; checkL.ZIndex=8
        ...
        itemBtn.MouseButton1Click:Connect(function()
            ...
            for i2,d in ipairs(itemFrames) do
                ...
                d.check.Text=isSel and "v" or ""
            end
            if onChange then onChange(ci) end; setOpen(false)
        end)
    end
    header.MouseButton1Click:Connect(function()
        ripple(header,header.AbsoluteSize.X*0.5,header.AbsoluteSize.Y*0.5,T.accent); setOpen(not open)
    end)
...
local function mkSubTabBar(parent,tabs)
    ...
    barContainer.Size=UDim2.new(1,0,0,BAR_H); barContainer.BackgroundColor3=Color3.fromRGB(11,10,18)
    ...
    for i,name in ipairs(tabs) do
        ...
        sbtn.MouseButton1Click:Connect(function()
            ripple(sbtn,sbtn.AbsoluteSize.X*0.5,sbtn.AbsoluteSize.Y*0.5,T.white); switchSub(ci)
        end)
    end
    return subPages
end

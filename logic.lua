...
-- =====================
-- ISLAND DATA
-- center = index titik terdekat dari centroid (untuk Farm V2)
-- =====================
local ISLANDS = {
    ["Starter Island"] = {
        coords = {
    ...
    ["Hollow"] = {
        ...
        }, center = 2,  -- dikoreksi dari centroid calc
    },
    -- Curse dihapus, diganti Shinjuku Island#2
    ["Shinjuku Island#1"] = {
        -- StrongSorcerer1-5 | center=4 (dist 3.33 dari centroid)
        coords = {
    ...
    ["Shinjuku Island#2"] = {
        -- Curse1-5 | center=3 (dist 3.43 dari centroid)
        coords = {
    ...
    ["Slime"] = {
        -- Slime1-5 | center=3 (dist 0.71 dari centroid — paling akurat)
        coords = {
    ...
    ["Academy"] = {
        -- AcademyTeacher1-5 | center=3 (dist 3.26 dari centroid)
        coords = {
    ...
    ["Judgement"] = {
        -- Swordsman1-5 | center=4 (dist 6.49 dari centroid)
        coords = {
    ...
    ["Soul Dominion"] = {
        -- Quincy1-5 | center=4 (dist 21.59 dari centroid)
        coords = {
    ...
    ["Ninja"] = {
        -- Ninja1-5 | center=4 (dist 3.02 dari centroid)
        coords = {
    ...
    ["Lawless"] = {
        -- ArenaFighter1-5 | center=2 (dist 7.27 dari centroid)
        coords = {
...
return function(refs, T)
    ...
    local function findNPC(radius)
        ...
        if not svc then return nil, nil end
            ...
            if npc then
                ...
                if pos then
                    ...
                    if d <= radius and d < bestD then
                        bestD = d; best = npc; bestName = "QuestNPC"..i
                    end
                end
            end
        end
    ...
    local function farmLoopV1()
        ...
        while farmV1On do
            ...
            for i, pos in ipairs(coords) do
                ...
                refs.setFarmStat(
                    island.." ["..i.."/"..#coords.."] "..(dist>100 and "Tween" or "TP"),
                    T.green)
    ...
    local function farmLoopV2()
        ...
        while farmV2On do
            ...
            local dist = r and (r.Position - fp).Magnitude or 0
            refs.setFarmStat(
                island.." [c:"..ci.."] "..(dist>100 and "Tween" or "TP"),
                T.green)
            ...
            if ld > 0 then
                ...
                while tick() < endT and farmV2On do
                    refs.setFarmStat(
                        island.." [c:"..ci.."] "..math.ceil(endT-tick()).."s",
                        T.green)
                    task.wait(0.1)
                end
    ...
    task.spawn(function()
        while task.wait(0.3) do
            if not questOn then
                refs.setQNPC("--"); lastFiredNPC = nil; continue
            end
            ...
            if name then
                ...
                if name ~= lastFiredNPC then
                    ...
                    fireSettings("DisablePVP",        true)
                    lastFiredNPC = name
                    refs.setQLast(name, T.accentGlow)
                end
            else
                refs.setQNPC("--", T.textDim)
            end
        end
    end)
    ...
    refs.setQuestCallback(function(v)
        ...
        if not v then
            lastFiredNPC = nil
            refs.setQNPC("--",   T.textDim)
            refs.setQLast("--",  T.textDim)
        end
    end)

    refs.setHitCallback(function(v)
        if v then
            startClick(refs.getCI())

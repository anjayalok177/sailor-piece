...
local function load(file)
    ...
    while attempts < 3 do
        ...
        lastErr = res
        warn("[YiDaMuSake] Gagal load "..file.." attempt "..attempts..": "..tostring(res))
...
pcall(function()
    local old = game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("YiDaMuSake")
    if old then old:Destroy() end
end)
pcall(function()
...
local function getViewport()
    ...
    local waited = 0
    while (vp.X == 0 or vp.Y == 0) and waited < 3 do
        task.wait(0.05); waited = waited + 0.05; vp = camera.ViewportSize
    end
...
rootGlow.Size              = UDim2.new(1, 100, 1, 100)
rootGlow.Position          = UDim2.new(0.5, 0, 0.5, 0)
rootGlow.AnchorPoint       = Vector2.new(0.5, 0.5)
rootGlow.BackgroundTransparency = 1
rootGlow.Image             = "rbxassetid://5028857084"
rootGlow.ImageColor3       = T.accent
rootGlow.ImageTransparency = 0.88
rootGlow.ZIndex            = 0
lib.regAccent("imgAccent", rootGlow)

local rootStroke = Instance.new("UIStroke", root)
rootStroke.Color        = T.border
...
task.spawn(function()
    local r = 125
    while bgGrad and bgGrad.Parent do r = r + 0.04; bgGrad.Rotation = r; task.wait(0.06) end
end)

local screenBlur = Instance.new("BlurEffect")
screenBlur.Size   = 0
screenBlur.Parent = camera
...
local function applyUIBgMode(mode)
    ...
    if     mode == "Solid"       then lib.smooth(bgF,{BackgroundTransparency=0},   0.3):Play(); lib.smooth(root,{BackgroundTransparency=0},   0.3):Play()
    elseif mode == "Transparent" then lib.smooth(bgF,{BackgroundTransparency=0.80},0.3):Play(); lib.smooth(root,{BackgroundTransparency=0.60},0.3):Play()
...
topFix.BackgroundColor3 = Color3.fromRGB(9, 8, 16)
topFix.BorderSizePixel  = 0; topFix.ZIndex = 5
local topSep = Instance.new("Frame", inner)
topSep.Size             = UDim2.new(1, 0, 0, 1)
topSep.Position         = UDim2.new(0, 0, 0, TOPBAR_H)
topSep.BackgroundColor3 = T.border
topSep.BackgroundTransparency = 0.1
...
iconImg.Size                    = UDim2.new(0.76, 0, 0.76, 0)
...
titleL.Size           = UDim2.new(0, 160, 0, 16); titleL.Position = UDim2.new(0, 50, 0, 7)
...
local subTitleL = Instance.new("TextLabel", topBar)
subTitleL.Size = UDim2.new(0, 160, 0, 12); subTitleL.Position = UDim2.new(0, 50, 0, 26)
...
local function mkWinBtn(offX, col, sym)
    ...
    b.Size = UDim2.new(0,18,0,18); b.Position = UDim2.new(1,offX,0.5,0); b.AnchorPoint = Vector2.new(1,0.5)
    b.BackgroundColor3 = col; b.Text = ""; b.BorderSizePixel = 0; b.ZIndex = 8
    ...
    b.MouseEnter:Connect(function() lib.smooth(b,{BackgroundTransparency=0.18},0.12):Play(); lib.smooth(lbl,{TextTransparency=0},0.12):Play() end)
    b.MouseLeave:Connect(function() lib.smooth(b,{BackgroundTransparency=0},0.12):Play(); lib.smooth(lbl,{TextTransparency=0.35},0.12):Play() end)
    return b
end
...
rh.Size = UDim2.new(0,20,0,20); rh.Position = UDim2.new(1,-2,1,-2); rh.AnchorPoint = Vector2.new(1,1)
...
miniGlow.Size = UDim2.new(1,34,1,34); miniGlow.Position = UDim2.new(0.5,0,0.5,0); miniGlow.AnchorPoint = Vector2.new(0.5,0.5)
miniGlow.BackgroundTransparency = 1; miniGlow.Image = "rbxassetid://5028857084"; miniGlow.ImageColor3 = T.accent; miniGlow.ImageTransparency = 0.84; miniGlow.ZIndex = 0
lib.regAccent("imgAccent", miniGlow)
local miniIconBg = Instance.new("Frame", miniBar)
miniIconBg.Size = UDim2.new(0,36,0,36); miniIconBg.Position = UDim2.new(0,5,0.5,0); miniIconBg.AnchorPoint = Vector2.new(0,0.5)
miniIconBg.BackgroundColor3 = T.accentSoft; miniIconBg.BorderSizePixel = 0; miniIconBg.ZIndex = 201
Instance.new("UICorner", miniIconBg).CornerRadius = UDim.new(0, 9); lib.regAccent("bgSoft", miniIconBg)
...
miniIconImg.Size = UDim2.new(0.82,0,0.82,0); miniIconImg.Position = UDim2.new(0.5,0,0.5,0); miniIconImg.AnchorPoint = Vector2.new(0.5,0.5)
miniIconImg.BackgroundTransparency = 1; miniIconImg.Image = "rbxassetid://110843044052526"; miniIconImg.ZIndex = 202
...
task.spawn(function()
    while miniBar and miniBar.Parent do
        if miniBar.Visible then lib.ease(miniGlow,{ImageTransparency=0.70},0.9):Play(); task.wait(1.0); lib.ease(miniGlow,{ImageTransparency=0.86},0.9):Play(); task.wait(1.0)
        else task.wait(0.5) end
    end
end)

closeBtn.MouseButton1Click:Connect(function()
    setBlur(0,0.18); lib.smooth(root,{Size=UDim2.new(0,WIN_W,0,0),BackgroundTransparency=1},0.22):Play()
    task.wait(0.24); gui:Destroy(); pcall(function() screenBlur:Destroy() end)
end)
minBtn.MouseButton1Click:Connect(function()
    miniBarVisible = true
    lib.smooth(root,{Size=UDim2.new(0,WIN_W,0,0),BackgroundTransparency=1},0.22):Play()
    ...
    lib.spring(miniBar,{Size=UDim2.new(0,228,0,46)},0.44):Play()
    task.spawn(function() task.wait(0.12); lib.smooth(miniBar,{Position=UDim2.new(0.5,-114,0,10)},0.20):Play() end)
    task.spawn(function()
        task.wait(0.20); if miniExpLbl then miniExpLbl:Destroy(); miniExpLbl = nil end
        miniExpLbl = Instance.new("TextLabel", miniBar)
        miniExpLbl.Size = UDim2.new(1,-50,1,0); miniExpLbl.Position = UDim2.new(0,46,0,0)
        ...
        miniExpLbl.TextTransparency = 1; miniExpLbl.ZIndex = 202; lib.smooth(miniExpLbl,{TextTransparency=0},0.24):Play()
    end)
end)
miniHit.MouseButton1Click:Connect(function()
    miniBarVisible = false; refreshBlur()
    if miniExpLbl then lib.smooth(miniExpLbl,{TextTransparency=1},0.12):Play(); task.wait(0.14); pcall(function() if miniExpLbl then miniExpLbl:Destroy(); miniExpLbl = nil end end) end
    lib.smooth(miniBar,{Size=UDim2.new(0,46,0,46),Position=UDim2.new(0.5,-23,0,10)},0.18):Play()
    task.wait(0.20); miniBar.Visible = false; root.Visible = true
    root.Size = UDim2.new(0,WIN_W,0,0); root.BackgroundTransparency = 1
    lib.spring(root,{Size=UDim2.new(0,WIN_W,0,WIN_H),BackgroundTransparency=0},0.42):Play()
    task.delay(0.5, function() applyUIBgMode(UISettings.uiBgMode) end)
end)

-- SIDEBAR
local sidebar = Instance.new("Frame", inner)
sidebar.Size = UDim2.new(0,SIDEBAR_W,1,-TOPBAR_H-1); sidebar.Position = UDim2.new(0,0,0,TOPBAR_H+1)
...
sideList.Size = UDim2.new(1,0,1,-8); sideList.Position = UDim2.new(0,0,0,10); sideList.BackgroundTransparency = 1; sideList.ZIndex = 5
local slL = Instance.new("UIListLayout", sideList)
slL.Padding = UDim.new(0,4); slL.SortOrder = Enum.SortOrder.LayoutOrder; slL.HorizontalAlignment = Enum.HorizontalAlignment.Center
...
local MAIN_TABS = {
    {name="Info", sym="i", tip="Info"}, {name="Main", sym="M", tip="Main"}, {name="Settings", sym="S", tip="Settings"},
}
local sideData = {}
local function switchMainTab(name)
    for _, d in pairs(sideData) do
        local on = (d.name == name)
        lib.smooth(d.iconBg,{BackgroundTransparency=on and 0 or 1},0.20):Play()
        lib.smooth(d.iconL,{TextColor3=on and T.white or T.textDim},0.20):Play()
        d.bar.Visible = on; d.page.Visible = on
    end
end
for i, tab in ipairs(MAIN_TABS) do
    local btn = Instance.new("TextButton", sideList)
    btn.Size = UDim2.new(1,0,0,52); btn.BackgroundTransparency = 1; btn.Text = ""; btn.BorderSizePixel = 0; btn.LayoutOrder = i; btn.ZIndex = 6
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 10)
    local bar = Instance.new("Frame", btn)
    bar.Size = UDim2.new(0,3,0.45,0); bar.Position = UDim2.new(0,-4,0.5,0); bar.AnchorPoint = Vector2.new(0,0.5)
    bar.BackgroundColor3 = T.accentGlow; bar.BorderSizePixel = 0; bar.Visible = false; bar.ZIndex = 7
    Instance.new("UICorner", bar).CornerRadius = UDim.new(1,0); lib.regAccent("bgGlow", bar)
    local iconBg2 = Instance.new("Frame", btn)
    iconBg2.Size = UDim2.new(0,40,0,40); iconBg2.Position = UDim2.new(0.5,0,0.5,0); iconBg2.AnchorPoint = Vector2.new(0.5,0.5)
    iconBg2.BackgroundColor3 = T.accentSoft; iconBg2.BackgroundTransparency = 1; iconBg2.BorderSizePixel = 0; iconBg2.ZIndex = 6
    Instance.new("UICorner", iconBg2).CornerRadius = UDim.new(0,10); lib.regAccent("bgSoft", iconBg2)
    local iconL = Instance.new("TextLabel", btn)
    iconL.Size = UDim2.new(1,0,1,0); iconL.BackgroundTransparency = 1; iconL.Text = tab.sym
    iconL.TextColor3 = T.textDim; iconL.Font = Enum.Font.GothamBold; iconL.TextSize = 20; iconL.ZIndex = 7
    local tooltip = Instance.new("TextLabel", btn)
    tooltip.Size = UDim2.new(0,70,0,22); tooltip.Position = UDim2.new(1,6,0.5,0); tooltip.AnchorPoint = Vector2.new(0,0.5)
    tooltip.BackgroundColor3 = Color3.fromRGB(20,18,32); tooltip.TextColor3 = T.text; tooltip.Text = tab.tip
    tooltip.Font = Enum.Font.GothamBold; tooltip.TextSize = 10; tooltip.Visible = false; tooltip.ZIndex = 50; tooltip.BorderSizePixel = 0
    ...
    btn.MouseLeave:Connect(function()
        ...
        if not isActive then lib.smooth(iconBg2,{BackgroundTransparency=1},0.14):Play() end
    end)
...
local botFix = Instance.new("Frame", botBar); botFix.Size = UDim2.new(1,0,0,14)
botFix.BackgroundColor3 = Color3.fromRGB(8,7,14); botFix.BorderSizePixel = 0; botFix.ZIndex = 5
...
dotL.BackgroundTransparency = 1; dotL.Text = "* online"; dotL.TextColor3 = T.green
dotL.Font = Enum.Font.GothamBold; dotL.TextSize = 9; dotL.TextXAlignment = Enum.TextXAlignment.Right; dotL.ZIndex = 6
task.spawn(function()
    while dotL and dotL.Parent do lib.ease(dotL,{TextColor3=T.green},0.9):Play(); task.wait(1.0); lib.ease(dotL,{TextColor3=T.greenDim},0.9):Play(); task.wait(1.0) end
end)
...
local ok, refs = pcall(buildPages,
...
if not ok then
    error("[YiDaMuSake] pages.lua gagal build: "..tostring(refs))
end

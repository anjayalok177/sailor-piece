...
-- =====================
-- SAFE LOADER
-- Retry 3x jika gagal, log error jelas
-- =====================
    ...
    while attempts < maxAttempts do
        ...
        local ok, res = pcall(function()
            ...
            if not src or src == "" then
                error("HttpGet returned empty for: " .. file)
            end
            local fn, compileErr = loadstring(src)
            if not fn then
                error("Compile error in " .. file .. ": " .. tostring(compileErr))
            end
            return fn()
        end)
        if ok then
            ...
            err = res
            warn("[YiDaMuSake] Load attempt " .. attempts .. " failed for " .. file .. ": " .. tostring(err))
            if attempts < maxAttempts then task.wait(1) end
        end
    end
    error("[YiDaMuSake] FATAL: Gagal load " .. file .. " setelah " .. maxAttempts .. " percobaan. Error: " .. tostring(err))
end
...
-- =====================
-- SCREEN GUI
-- Fallback: jika CoreGui gagal, pakai PlayerGui
-- =====================
local gui
local function createGui()
    ...
    if not ok then
        warn("[YiDaMuSake] CoreGui gagal, fallback ke PlayerGui: " .. tostring(err))
        gui = Instance.new("ScreenGui")
        gui.Name           = "YiDaMuSake"
        gui.ResetOnSpawn   = false
        gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
-- =====================
-- VIEWPORT SIZE
-- Tunggu sampai kamera benar-benar ready
-- Mencegah WIN_W/WIN_H = 0
-- =====================
local camera = game.Workspace.CurrentCamera
local function getViewport()
    local vp = camera.ViewportSize
    -- Jika belum ready (0,0), tunggu max 3 detik
    local waited = 0
    ...
    -- Fallback hardcode jika masih 0 (portrait/landscape iPhone)
    if vp.X == 0 or vp.Y == 0 then
        warn("[YiDaMuSake] ViewportSize masih 0, pakai fallback 812x375")
        return Vector2.new(812, 375)
    end
    return vp
end
...
task.spawn(function()
    local r = 125
    while bgGrad and bgGrad.Parent do
        r = r + 0.04; bgGrad.Rotation = r; task.wait(0.06)
    end
end)
...
titleL.Size            = UDim2.new(0, 160, 0, 16)
titleL.Position        = UDim2.new(0, 50, 0, 7)
titleL.BackgroundTransparency = 1
titleL.Text            = "Yi Da Mu Sake"
...
subTitleL.Size           = UDim2.new(0, 160, 0, 12)
subTitleL.Position       = UDim2.new(0, 50, 0, 26)
subTitleL.BackgroundTransparency = 1
subTitleL.Text           = "sailor piece  v8.2"
subTitleL.TextColor3     = T.textDim
...
local function mkWinBtn(offX, col, sym)
    ...
    b.Position        = UDim2.new(1, offX, 0.5, 0)
...
rh.BackgroundColor3  = Color3.fromRGB(22, 20, 34)
rh.BackgroundTransparency = 0.4
rh.Text              = ""
...
for di = 1, 3 do
    ...
    dot.Position        = UDim2.new(0, 2+di*4, 0, 2+di*4)
    dot.BackgroundColor3 = T.accentGlow
...
do
    ...
    rh.MouseButton1Down:Connect(function()
        ...
        rStartW  = root.AbsoluteSize.X
        rStartH  = root.AbsoluteSize.Y
    end)
    UIS.InputChanged:Connect(function(i)
...
miniBar.Position        = UDim2.new(0.5, -23, 0, 10)
miniBar.BackgroundColor3 = Color3.fromRGB(11, 10, 18)
...
miniGlow.ImageColor3     = T.accent
miniGlow.ImageTransparency = 0.84
miniGlow.ZIndex          = 0
lib.regAccent("imgAccent", miniGlow)

local miniIconBg = Instance.new("Frame", miniBar)
miniIconBg.Size            = UDim2.new(0, 36, 0, 36)
miniIconBg.Position        = UDim2.new(0, 5, 0.5, 0)
miniIconBg.AnchorPoint     = Vector2.new(0, 0.5)
miniIconBg.BackgroundColor3 = T.accentSoft
miniIconBg.BorderSizePixel  = 0
miniIconBg.ZIndex           = 201
...
miniIconImg.Size            = UDim2.new(0.82, 0, 0.82, 0)
miniIconImg.Position        = UDim2.new(0.5, 0, 0.5, 0)
miniIconImg.AnchorPoint     = Vector2.new(0.5, 0.5)
miniIconImg.BackgroundTransparency = 1
miniIconImg.Image           = "rbxassetid://110843044052526"
miniIconImg.ZIndex          = 202
...
task.spawn(function()
    while miniBar and miniBar.Parent do
        if miniBar.Visible then
            lib.ease(miniGlow, {ImageTransparency=0.70}, 0.9):Play(); task.wait(1.0)
            lib.ease(miniGlow, {ImageTransparency=0.86}, 0.9):Play(); task.wait(1.0)
...
minBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        task.wait(0.12)
        lib.smooth(miniBar, {Position=UDim2.new(0.5, -114, 0, 10)}, 0.20):Play()
    end)
    task.spawn(function()
        task.wait(0.20)
        ...
        miniExpLbl.TextTransparency = 1
        miniExpLbl.ZIndex          = 202
        lib.smooth(miniExpLbl, {TextTransparency=0}, 0.24):Play()
    end)
end)
...
miniHit.MouseButton1Click:Connect(function()
    ...
    if miniExpLbl then
        lib.smooth(miniExpLbl, {TextTransparency=1}, 0.12):Play()
        task.wait(0.14)
        pcall(function() if miniExpLbl then miniExpLbl:Destroy(); miniExpLbl = nil end end)
    end
...
sideFix.Size            = UDim2.new(0, 14, 1, 0)
sideFix.Position        = UDim2.new(1, -14, 0, 0)
sideFix.BackgroundColor3 = Color3.fromRGB(9, 8, 16)
sideFix.BorderSizePixel  = 0
sideFix.ZIndex          = 4

local sideVLine = Instance.new("Frame", inner)
sideVLine.Size            = UDim2.new(0, 1, 1, -TOPBAR_H-1)
sideVLine.Position        = UDim2.new(0, SIDEBAR_W, 0, TOPBAR_H+1)
sideVLine.BackgroundColor3 = T.border
sideVLine.BackgroundTransparency = 0.1
sideVLine.BorderSizePixel  = 0
sideVLine.ZIndex          = 6
...
slP.PaddingLeft  = UDim.new(0, 6)
slP.PaddingRight = UDim.new(0, 6)
...
-- =====================
local MAIN_TABS = {
    {name="Info",     sym="i", tip="Info"},
    {name="Main",     sym="M", tip="Main"},
    {name="Settings", sym="S", tip="Settings"},
}
local sideData = {}
...
for i, tab in ipairs(MAIN_TABS) do
    ...
    iconBg2.Size            = UDim2.new(0, 40, 0, 40)
    iconBg2.Position        = UDim2.new(0.5, 0, 0.5, 0)
    iconBg2.AnchorPoint     = Vector2.new(0.5, 0.5)
    iconBg2.BackgroundColor3 = T.accentSoft
    iconBg2.BackgroundTransparency = 1
    iconBg2.BorderSizePixel  = 0
    iconBg2.ZIndex          = 6
    ...
    tooltip.Position        = UDim2.new(1, 6, 0.5, 0)
    tooltip.AnchorPoint     = Vector2.new(0, 0.5)
    tooltip.BackgroundColor3 = Color3.fromRGB(20, 18, 32)
    ...
    btn.MouseEnter:Connect(function()
        ...
        lib.smooth(iconBg2, {BackgroundTransparency=0.85}, 0.14):Play()
    end)
...
botBar.Size            = UDim2.new(1, 0, 0, BOTBAR_H)
botBar.Position        = UDim2.new(0, 0, 1, -BOTBAR_H)
botBar.BackgroundColor3 = Color3.fromRGB(8, 7, 14)
botBar.BorderSizePixel  = 0
botBar.ZIndex          = 5

local botFix = Instance.new("Frame", botBar)
botFix.Size            = UDim2.new(1, 0, 0, 14)
botFix.BackgroundColor3 = Color3.fromRGB(8, 7, 14)
botFix.BorderSizePixel  = 0
botFix.ZIndex          = 5
Instance.new("UICorner", botBar).CornerRadius = UDim.new(0, 14)
Instance.new("UIStroke", botBar).Color        = T.border

local verL = Instance.new("TextLabel", botBar)
verL.Size            = UDim2.new(0.5, 0, 1, 0)
verL.Position        = UDim2.new(0, 12, 0, 0)
verL.BackgroundTransparency = 1
verL.Text            = "sailor piece  v8.2 | Anti-AFK ON"
verL.TextColor3      = T.textDim
...
dotL.Size            = UDim2.new(0.5, -12, 1, 0)
dotL.Position        = UDim2.new(0.5, 0, 0, 0)
dotL.BackgroundTransparency = 1
dotL.Text            = "* online"
dotL.TextColor3      = T.green
dotL.Font            = Enum.Font.GothamBold
dotL.TextSize        = 9
dotL.TextXAlignment  = Enum.TextXAlignment.Right
dotL.ZIndex          = 6

task.spawn(function()
    while dotL and dotL.Parent do
        lib.ease(dotL, {TextColor3=T.green},    0.9):Play(); task.wait(1.0)
        lib.ease(dotL, {TextColor3=T.greenDim}, 0.9):Play(); task.wait(1.0)
    end
end)

-- =====================
-- ENTRANCE ANIMATION
-- Dipastikan root visible dulu sebelum spring
-- =====================
root.BackgroundTransparency = 1
root.Size     = UDim2.new(0, WIN_W, 0, 0)
root.Position = UDim2.new(0.5, -WIN_W/2, 0.5, -WIN_H/2)
root.Visible  = true  -- pastikan visible
...
-- =====================
-- LOAD PAGES + LOGIC
-- Dibungkus pcall agar error tidak silent
-- =====================
...
_G.YiUI = refs
print("[YiDaMuSake] v8.2 loaded sukses! UI parent: " .. gui.Parent.Name)

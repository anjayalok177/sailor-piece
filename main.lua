...
local SIDEBAR_W = 68; local TOPBAR_H = 50; local BOTBAR_H = 26
...
-- =====================
-- TOPBAR (modern — icon-only sidebar reference style)
-- =====================
...
iconBg.Size = UDim2.new(0,30,0,30); iconBg.Position = UDim2.new(0,12,0.5,0)
...
subTitleL.BackgroundTransparency = 1; subTitleL.Text = "sailor piece  •  v7"
subTitleL.TextColor3 = T.textDim; subTitleL.Font = Enum.Font.Gotham
subTitleL.TextSize = 9; subTitleL.TextXAlignment = Enum.TextXAlignment.Left; subTitleL.ZIndex = 7

-- Search bar (center-right of topbar)
local searchFrame = Instance.new("Frame",topBar)
searchFrame.Size = UDim2.new(0,148,0,28); searchFrame.Position = UDim2.new(0.5,-40,0.5,0)
searchFrame.AnchorPoint = Vector2.new(0,0.5)
searchFrame.BackgroundColor3 = Color3.fromRGB(14,13,22)
...
searchIcon.Size = UDim2.new(0,24,1,0); searchIcon.BackgroundTransparency = 1
searchIcon.Text = "⌕"; searchIcon.TextColor3 = T.textDim
searchIcon.Font = Enum.Font.GothamBold; searchIcon.TextSize = 15; searchIcon.ZIndex = 9
...
searchBox.TextSize = 11; searchBox.TextXAlignment = Enum.TextXAlignment.Left
searchBox.ZIndex = 9; searchBox.ClearTextOnFocus = false
searchFrame.MouseEnter:Connect(function()
    lib.smooth(searchStroke,{Color=T.accentGlow,Transparency=0.05},0.14):Play()
end)
searchFrame.MouseLeave:Connect(function()
    lib.smooth(searchStroke,{Color=T.border,Transparency=0.2},0.14):Play()
end)

-- Win buttons
local function mkWinBtn(offX,col,sym)
    local b=Instance.new("TextButton",topBar)
    b.Size=UDim2.new(0,18,0,18); b.Position=UDim2.new(1,offX,0.5,0)
    ...
    lbl.TextSize=10; lbl.TextTransparency=0.35; lbl.ZIndex=9
    b.MouseEnter:Connect(function()
        lib.smooth(b,{BackgroundTransparency=0.18},0.12):Play()
        lib.smooth(lbl,{TextTransparency=0},0.12):Play()
    end)
    b.MouseLeave:Connect(function()
        lib.smooth(b,{BackgroundTransparency=0},0.12):Play()
        lib.smooth(lbl,{TextTransparency=0.35},0.12):Play()
    end)
    return b
end
...
rh.Name="ResizeHandle"; rh.Size=UDim2.new(0,20,0,20)
...
for di=1,3 do
    ...
    dot.BackgroundColor3=T.accentGlow; dot.BackgroundTransparency=0.4
...
-- =====================
-- MINIMIZE BAR (forward declaration pattern — FIX)
-- =====================
local miniExpLbl=nil

miniBar = Instance.new("Frame",gui)   -- assign to forward-declared local
miniBar.Size     = UDim2.new(0,46,0,46)
miniBar.Position = UDim2.new(0.5,-23,0,10)
miniBar.BackgroundColor3 = Color3.fromRGB(11,10,18)
miniBar.BorderSizePixel  = 0; miniBar.ZIndex = 200; miniBar.Visible = false
Instance.new("UICorner",miniBar).CornerRadius = UDim.new(0,23)
local miniStroke=Instance.new("UIStroke",miniBar)
miniStroke.Color=T.borderBright; miniStroke.Thickness=1.5; miniStroke.Transparency=0.12

local miniGlow=Instance.new("ImageLabel",miniBar)
miniGlow.Size=UDim2.new(1,34,1,34); miniGlow.Position=UDim2.new(0.5,0,0.5,0)
...
miniGlow.ImageTransparency=0.84; miniGlow.ZIndex=0
...
minBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        miniExpLbl.TextColor3=T.text; miniExpLbl.Font=Enum.Font.GothamBold
        miniExpLbl.TextSize=13; miniExpLbl.TextXAlignment=Enum.TextXAlignment.Center
...
miniHit.MouseButton1Click:Connect(function()
    ...
    task.wait(0.20); miniBar.Visible=false
    root.Visible=true; root.Size=UDim2.new(0,WIN_W,0,0); root.BackgroundTransparency=1
...
-- =====================
-- SIDEBAR TABS (icon-only, modern style)
-- =====================
local MAIN_TABS={
    {name="Info",     sym="ℹ",  tip="Info"},
...
local function switchMainTab(name)
    for _,d in pairs(sideData) do
        ...
        -- Active: purple filled bg, bright icon
        -- Inactive: transparent bg, dim icon
...
for i,tab in ipairs(MAIN_TABS) do
    ...
    btn.Size=UDim2.new(1,0,0,52); btn.BackgroundTransparency=1
    ...
    -- Icon background (active state = purple, inactive = transparent)
    local iconBg2=Instance.new("Frame",btn)
    iconBg2.Size=UDim2.new(0,40,0,40); iconBg2.Position=UDim2.new(0.5,0,0.5,0)
    ...
    tooltip.Size=UDim2.new(0,70,0,22); tooltip.Position=UDim2.new(1,6,0.5,0)
    tooltip.AnchorPoint=Vector2.new(0,0.5); tooltip.BackgroundColor3=Color3.fromRGB(20,18,32)
    tooltip.TextColor3=T.text; tooltip.Text=tab.tip
    tooltip.Font=Enum.Font.GothamBold; tooltip.TextSize=10
    tooltip.Visible=false; tooltip.ZIndex=50; tooltip.BorderSizePixel=0
    ...
    btn.MouseLeave:Connect(function()
        ...
        -- Only hide hover bg if not active
        local isActive=(sideData[tab.name] and sideData[tab.name].bar.Visible)
        if not isActive then
            lib.smooth(iconBg2,{BackgroundTransparency=1},0.14):Play()
        end
    end)
    ...
    if i<#MAIN_TABS then
        ...
        sep.BackgroundColor3=T.border; sep.BackgroundTransparency=0.4
...
-- Filter top-level card frames in scroll pages by text content
-- =====================
local function doSearch(query)
    query=query:lower()
    for _,tabData in pairs(sideData) do
                                ...
                                if query=="" then
                                    ...
                                    for _,desc in ipairs(card:GetDescendants()) do
                                        if (desc:IsA("TextLabel") or desc:IsA("TextButton"))
                                            and desc.Text:lower():find(query,1,true) then
                                            hasMatch=true; break
                                        end
                                    end
                                    card.Visible=hasMatch
                                end
                            end
                        end
...
botFix.Size=UDim2.new(1,0,0,14); botFix.BackgroundColor3=Color3.fromRGB(8,7,14)
...
verL.BackgroundTransparency=1; verL.Text="sailor piece  v7"
...
task.wait(0.08)
lib.spring(root,{Size=UDim2.new(0,WIN_W,0,WIN_H),BackgroundTransparency=0},0.50):Play()
switchMainTab("Info")

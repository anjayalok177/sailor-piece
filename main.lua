...
-- Cleanup blur effect lama kalau ada
pcall(function()
...
-- =====================
-- BLUR MANAGEMENT (satu BlurEffect, dikelola satu tempat)
-- BlurEffect hanya memblur dunia 3D di belakang UI,
-- elemen ScreenGui tetap tajam → efek "frosted glass" iPhone
-- =====================
local screenBlur = Instance.new("BlurEffect")
screenBlur.Size = 0
screenBlur.Parent = game.Workspace.CurrentCamera

local miniBarVisible = false  -- track state minimize

local function setBlur(targetSize, duration)
    TweenService:Create(screenBlur,
        TweenInfo.new(duration or 0.35, Enum.EasingStyle.Quint),
        {Size = targetSize}):Play()
end

local function refreshBlur()
    -- Prioritas: miniBar blur > UI window blur > no blur
    if miniBarVisible and UISettings.miniBgMode == "Blur" then
        setBlur(12)        -- blur ringan saat minimize (frosted glass)
    elseif root.Visible and UISettings.uiBgMode == "Blur" then
        setBlur(18)        -- blur lebih kuat saat window terbuka
    else
        setBlur(0)         -- tidak ada blur
    end
end
...
local function applyMiniBgMode(mode)
    ...
    elseif mode == "Blur" then
        -- semi-transparan agar dunia 3D yang diblur terlihat di baliknya
...
-- miniBar harus dibuat SEBELUM applyMiniBgMode dipanggil
...
minBtn.MouseButton1Click:Connect(function()
    minimized = true; miniBarVisible = true
    lib.smooth(root, {Size=UDim2.new(0,WIN_W,0,0), BackgroundTransparency=1}, 0.24):Play()
    task.wait(0.20); root.Visible = false
    -- Apply BG mode miniBar
    applyMiniBgMode(UISettings.miniBgMode)
    refreshBlur()   -- aktifkan blur jika mode Blur
    miniBar.Size = UDim2.new(0,46,0,46)
    miniBar.Position = UDim2.new(0.5,-23,0,8)
...
miniHit.MouseButton1Click:Connect(function()
    minimized = false; miniBarVisible = false
    -- Matikan blur miniBar segera
    refreshBlur()
    if miniExpLbl then
    ...
    lib.smooth(miniBar,
        ...
        0.20):Play()
    task.wait(0.18); miniBar.Visible = false
    root.Visible = true
    ...
    -- Re-apply UI bg mode setelah window terbuka

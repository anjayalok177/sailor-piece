...
-- Anti-AFK menggunakan VirtualUser (metode terbaik, tidak mengganggu kontrol)
...
local gui=Instance.new("ScreenGui"); gui.Name="YiDaMuSake"; gui.ResetOnSpawn=false
gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; gui.IgnoreGuiInset=true; gui.Parent=game:GetService("CoreGui")
...
rootGlow.ImageTransparency=0.88; rootGlow.ZIndex=0; lib.regAccent("imgAccent",rootGlow)
...
local inner=Instance.new("Frame",root); inner.Size=UDim2.new(1,0,1,0)
inner.BackgroundTransparency=1; inner.ClipsDescendants=true; inner.ZIndex=1
...
local function spawnParticles(count)
    for _,p in ipairs(particleList) do pcall(function() p:Destroy() end) end; particleList={}
...
-- TOPBAR
local topBar=Instance.new("Frame",inner); topBar.Name="TopBar"; topBar.Size=UDim2.new(1,0,0,TOPBAR_H)
...
Instance.new("UICorner",iconBg).CornerRadius=UDim.new(0,8); lib.regAccent("bgSoft",iconBg)
...
subTitleL.BackgroundTransparency=1; subTitleL.Text="sailor piece  v8.1"; subTitleL.TextColor3=T.textDim
...
-- RESIZE
local rh=Instance.new("TextButton",root); rh.Name="ResizeHandle"
...
miniGlow.Image="rbxassetid://5028857084"; miniGlow.ImageColor3=T.accent; miniGlow.ImageTransparency=0.84; miniGlow.ZIndex=0
...
-- SIDEBAR
local sidebar=Instance.new("Frame",inner); sidebar.Name="Sidebar"
...
for i,tab in ipairs(MAIN_TABS) do
    ...
    tooltip.TextColor3=T.text; tooltip.Text=tab.tip; tooltip.Font=Enum.Font.GothamBold; tooltip.TextSize=10
...
local botFix=Instance.new("Frame",botBar); botFix.Size=UDim2.new(1,0,0,14); botFix.BackgroundColor3=Color3.fromRGB(8,7,14)
botFix.BorderSizePixel=0; botFix.ZIndex=5; Instance.new("UICorner",botBar).CornerRadius=UDim.new(0,14)
...
verL.BackgroundTransparency=1; verL.Text="sailor piece  v8.1 | Anti-AFK ON"
verL.TextColor3=T.textDim; verL.Font=Enum.Font.Gotham; verL.TextSize=9; verL.TextXAlignment=Enum.TextXAlignment.Left; verL.ZIndex=6
...
-- ENTRANCE
root.BackgroundTransparency=1; root.Size=UDim2.new(0,WIN_W,0,0); task.wait(0.08)
lib.spring(root,{Size=UDim2.new(0,WIN_W,0,WIN_H),BackgroundTransparency=0},0.50):Play()
switchMainTab("Info")

-- LOAD PAGES + LOGIC
local buildPages=load("pages.lua")
local refs=buildPages(lib,sideData,contentArea,bgF,root,rootCorner,rootStroke,rootGlow,particleList,spawnParticles,applyUIBgMode,applyMiniBgMode,gui)
